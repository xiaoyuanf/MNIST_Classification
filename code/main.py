...
#    if question == "1":
    ...
    if Model == "knn":
            ...
            for a in range(3):
                ...
                for train, validate in kf.split(X, y):
                    model.fit(X[train], y[train], epoch=100, 
                          ...
                          alpha=alpha[a])
                    #print("When batch size is", minibatch[m], 
                    #  "alpha is", alpha[a], "training error is %.3f" % 
        ...
        #model.fit(X, y, epoch=100, minibatch=1000, alpha=0.001)
        ...
        #print("Training error %.3f" % utils.classification_error(model.predict(X), y))
        #print("Validation error %.3f" % utils.classification_error(model.predict(Xvalid), yvalid))
        #print("Test error %.3f" % utils.classification_error(model.predict(Xtest), ytest))
...
#                      utils.classification_error(model.predict(X), y))
        ...
        model.fit(X, y, epoch=100, minibatch=500, alpha=0.0001)
