...
def log_sum_exp(Z):
    ...
    # [:,None] create a new axis with length 1
    # Z - Z_max[:,None], each element of a row minus the max element of the row
    return Z_max + np.log(np.sum(np.exp(Z - Z_max[:,None]), axis=1)) # per-colmumn max

class NeuralNet():
    # uses sigmoid nonlinearity
    def __init__(self, hidden_layer_sizes, lammy=1, max_iter=100, alpha=0.001):
